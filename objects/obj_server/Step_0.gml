...
if count > 0
{
	...
	with(obj_player)
	{
		...
		buffer_write(global.player_buffer, buffer_s16, gun.image_angle);
		
		////Send bullets, loop through bullet list, and write the size of it
		////Write total number of bullets
		////Add bullet coordinates
		buffer_write(global.player_buffer, buffer_s16, ds_list_size(gun.bullets));
		
		for(var i = 0; i < ds_list_size(gun.bullets); i++){
			var bullet = ds_list_find_value(gun.bullets, i);
			
			buffer_write(global.player_buffer, buffer_s16, bullet.x);
			buffer_write(global.player_buffer, buffer_s16, bullet.y);
		}
	}
